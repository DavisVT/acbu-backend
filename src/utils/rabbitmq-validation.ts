 ...
 *
 * Zod parses — and therefore *validates* — the payload; if a numeric field
 * arrives as a string (or any other type mismatch) Zod will throw a ZodError,
 * which is converted to a `MessageValidationError` and logged before
 * re-throwing.
 *
 * `T` is supplied by the caller, so this signature cannot tie a payload to the
 * queue it belongs to — the schema is taken from the runtime queue name and
 * the parsed result is only *described* as `T`. New code should prefer
 * `validateQueueMessage`, where the queue name determines the payload type and
 * the two cannot disagree.
 */
export function validateMessage<T>(queue: string, payload: unknown): T {
  const schema: z.ZodTypeAny | undefined = QUEUE_SCHEMAS[queue as keyof typeof QUEUE_SCHEMAS];
  ...
  try {
    // A `ZodTypeAny` parses to `any`, so the parsed value is returned as `T`
    // without asserting that the schema produces `T`.
